...
# Active theme (mutable dict, updated on switch)
...
class Btn(tk.Frame):
    def __init__(self, parent, text, cmd, w=50, h=22,
                 bg=None, bgh=None, bgp=None,
                 fg=None, font=None, fbg=None):
        bg  = bg  or T["C_BTN"]
...
class AskDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        tk.Label(f, text="Project name", fg=T["C_TEXT_HEAD"], bg=T["C_BG"],
                 font=T["FN"]).pack(anchor="w", pady=(0,8))
        self._e = tk.Entry(f, bg=T["C_HEADER"], fg=T["C_TEXT"],
                           insertbackground=T["C_ACCENT"],
                           relief="flat", font=T["FS"], bd=0,
                           highlightthickness=1,
        ...
        Btn(bf,"Add",self._ok,w=72,h=26,
            ...
            fg="#001830" if T.get("name")=="2007 Aesthetic" else "#1a1a1a",
            fbg=T["C_BG"]).pack(side="right")
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent, current_theme, on_change):
        super().__init__(parent)
        self.title("Settings")
        self.configure(bg=T["C_BG"])
        self.resizable(False, False)
        ...
        tk.Frame(self, bg=T["C_ACCENT"], height=2).pack(fill="x")
        f = tk.Frame(self, bg=T["C_BG"], padx=20, pady=16); f.pack(fill="both")

        tk.Label(f, text="Settings", fg=T["C_TEXT_HEAD"], bg=T["C_BG"],
                 font=T["FN"]).pack(anchor="w", pady=(0,14))
        tk.Frame(f, bg=T["C_BORDER"], height=1).pack(fill="x", pady=(0,12))

        tk.Label(f, text="Theme", fg=T["C_TEXT_DIM"], bg=T["C_BG"],
                 font=T["FT"]).pack(anchor="w", pady=(0,6))

        self._var = tk.StringVar(value=current_theme)

        for key, theme in THEMES.items():
            row = tk.Frame(f, bg=T["C_BG"], cursor="hand2")
            row.pack(fill="x", pady=2)

            rb = tk.Radiobutton(row, variable=self._var, value=key,
                                bg=T["C_BG"],
                                activebackground=T["C_BG"],
                                selectcolor=T["C_HEADER"],
                                fg=T["C_ACCENT"],
                                cursor="hand2",
                                command=lambda k=key: self._pick(k))
            rb.pack(side="left")
            tk.Label(row, text=theme["name"],
                     fg=T["C_TEXT"], bg=T["C_BG"], font=T["FS"],
    ...
    def _ctr(self,p):
        self.update_idletasks()
        x=p.winfo_x()+p.winfo_width()-self.winfo_width()-10
        y=p.winfo_y()+50
        self.geometry(f"+{x}+{y}")
...
class Tracker(tk.Tk):
    def __init__(self):
        ...
        self._apply_theme(self._settings.get("theme", "default"), rebuild=False)
    ...
    def _apply_theme(self, key, rebuild=True):
        global T
    ...
    def _build_ui(self):
        ...
        tk.Label(self, textvariable=self._sv, fg=T["C_TEXT_DIM"], bg=T["C_HEADER"],
                 font=T["FT"], anchor="w", padx=10, pady=3).pack(fill="x")
    ...
    def _row(self, idx, proj, alt):
        ...
        sc     = T["C_GREEN"] if status=="green" else (T["C_YELLOW"] if status=="yellow" else T["C_RED_LED"])
        sl     = "working" if active else ("active" if status=="green" else ("idle" if status=="yellow" else "stalled"))
        ...
        si = tk.Frame(c1, bg=bbg)
        si.pack(side="left", padx=10, pady=0, fill="y")
        # Centre vertically with a sub-frame
        sc_f = tk.Frame(si, bg=bbg)
        sc_f.pack(expand=True, fill="both")
        led_c = make_led(sc_f, sc, size=10, bg=bbg)
        led_c.pack(side="left", pady=12)
        tk.Label(sc_f, text=sl, fg=sc, bg=bbg, font=T["FS"], padx=4).pack(side="left")
        hw += [c1, si, sc_f]

        # ── Col 3: Session/last worked ─────────────────────────────────────
        ...
        wl = tk.Label(c3, textvariable=wv, fg=T["C_TEXT_DIM"], bg=bbg,
                      font=T["FS"], anchor="w", padx=10)
        ...
        # ── Buttons ────────────────────────────────────────────────────────
        bf = tk.Frame(row, bg=bbg, padx=6)
        bf.pack(side="right")
        hw.append(bf)

        if active:
            Btn(bf, "End", lambda i=idx: self._end(i),
                w=42, h=22, bg=T["C_RED"], bgh=T["C_RED_H"], bgp=T["C_RED_P"],
                fg=T["C_TEXT"], font=T["FT"], fbg=bbg).pack(side="left", padx=(0,4))
        else:
            Btn(bf, "Start", lambda i=idx: self._start(i),
                w=42, h=22, fbg=bbg, font=T["FT"]).pack(side="left", padx=(0,4))

        Btn(bf, "×", lambda i=idx: self._delete(i),
            w=22, h=22, bg=T["C_BTN"], bgh=T["C_RED_H"], bgp=T["C_RED_P"],
    ...
    def _detail(self, idx, proj):
        ...
        if not breakdown:
            tk.Label(panel, text="No sessions yet.",
                     fg=T["C_TEXT_DIM"], bg=T["C_EXP_BG"],
                     font=T["FS"], pady=8, padx=20).pack(anchor="w")
            return
        ...
        for day_dt, secs in breakdown:
            ...
            r = tk.Frame(panel, bg=T["C_EXP_BG"], padx=20, pady=2)
            r.pack(fill="x")
    ...
    def _open_settings(self):
        SettingsDialog(self, self._settings.get("theme","default"), self._apply_theme)
